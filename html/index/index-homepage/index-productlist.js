...
//									_this.switchShowType(tradeObj).interestSec + '<span class="app_f24">%</span></span></div><p class="app_f14 app_mt10">昨日最高年化收益</p></div><div class="mui-col-xs-6 app_f14 app_mt25 app_mb10 app_fl text-align-center app_ca6"><span class="app_c6">起投金额：</span>'+ GHUTILS.formatCurrency(tradeObj.investMin) +'元</div><div class="mui-col-xs-6 app_fl app_f14 app_mt25 app_mb10 text-align-center app_ca6"><span class="app_c6">锁定期：</span>灵活赎回</div><div class="app_mt25"><button type="button" class="mui-btn mui-btn-primary mui-btn-block">'+buttonHtml+'</button></div></div></div>';	
...
//				var collectPercent = GHUTILS.toFixeds(GHUTILS.Fmul(GHUTILS.Fdiv(GHUTILS.Fadd(tradeObj.collectedVolume, GHUTILS.Fmul(tradeObj.lockCollectedVolume, 1)), tradeObj.raisedTotalNumber), 100),2);
				var type = '', soldOut = '', buttonHtml = '立即抢购', detailtype = ' app-recommend-dingqi';
				if(!tradeObj.investMin){
					tradeObj.investMin = 0
				}
...
//					}
				...
				$(".app_soldoutdetail").off().on("tap", function() {
					if(GHUTILS.linkChackLogin(this.id, ws.id)){
						_this.protnlist($(this).attr("data-href"));
					}
				})
			},
...
//								mui.toast("募集结束")
							...
							if(GHUTILS.isLabelProduct(result.productLabels, "1", true)){
								GHUTILS.isFreshman(function(){
									_this.goOrder(thisId, wsId, result);
								})
							}else{
								_this.goOrder(thisId, wsId, result);
							}
			...
			goOrder: function(thisId, wsId, result){
				if (GHUTILS.linkChackLogin(thisId, wsId)) {
					if(!result.investMin){
						result.investMin = 0
					}
					if(!result.investMax){
					...
					GHUTILS.OPENPAGE({
						url: "../product-t0/product-t0-order.html",
