		...
		getList: function(index, refresh) {
			...
			GHUTILS.LOAD({
				...
				callback: function(result) {
					...
					if (GHUTILS.checkErrorCode(result)&&GHUTILS.getloginStatus(true)) {
						if (_this.pageNum[index] == 1) {
							$('#app-' + _this.states[index] + '-num').html('(' + result.total + ')');
						}
//					console.log(_this.states[index]+JSON.stringify(result))
		...
		bindEvent: function() {
			var _this = this
			$(".app_user_coupon_box.on .coupon_receive_btn").off().on("tap", function(){
			...
			$(".tasteCoupon_btn").off().on("tap", function(){
