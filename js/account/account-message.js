...
//			$("#app_mail_box").css({"top": GHUTILS.setTop(44)+"px"})
			this.pageInit(); //页面初始化
			this.bindEvent(); //事件绑定
			this.pullRefresh();
		},
		pageInit: function() {
			var _this = this;
			if(_this.ws.topage && _this.ws.topage == "mail"){
				$("#app_title").html("站内信")
...
//			if (mui("#app_mail_box").slider().getSlideNumber() == index) {
...
//			var html="<div class='app_myproduct_box app_pb20 app_links' data-title='"+ Obj.title +"' data-url='"+ Obj.url +"'><div class='app_c5'>"+ Obj.title +"<span class='mui-pull-right app_c9'>"+ time +"</span></div><div class='app_cb3 mui-ellipsis-2'>"+ encodeURIComponent(Obj.summary) +"</div></div>"
			var html="<div class='app_myproduct_box app_pb20 app_links' data-type='"+ Obj.type +"' data-title='"+ Obj.title +"' data-url='"+ Obj.url +"' data-summary='"+ encodeURIComponent(Obj.summary) +"'><div class='app_c3b app_lh24 app_f15'>"+ Obj.title +"<span class='mui-pull-right app_ca6 app_f12'>"+ time +"</span></div><div class='app_ca6 app_f13 mui-ellipsis-2 app_lh24'>"+ Obj.summary +"</div></div>"
			return html;
		},
					...
					if(GHUTILS.checkErrorCode(result)){
						GHUTILS.OPENPAGE({
							...
							extras:{
								...
								content: encodeURIComponent(result.mesContent)
							}
						})
					}
