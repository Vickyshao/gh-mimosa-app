...
(function($) {
	...
	PR.prototype = {
		...
		getNotice: function(page, rows, refresh) {
			var _this = this;
			var num = plus.screen.resolutionWidth < 375 ? 8 : 12;
			GHUTILS.LOAD({
					...
					$.each(notice, function(i, val) {
						...
						if(val.linkUrl){
							...
							if(val.linkHtml){
								...
								linkHtml = encodeURIComponent(val.linkHtml)
								if(val.linkHtml.lastIndexOf(",") != -1 && val.linkHtml.length - val.linkHtml.lastIndexOf(",") == 1){
									linkHtml = encodeURIComponent(val.linkHtml.substr(0,val.linkHtml.lastIndexOf(",")))
								}
							}
						}
