...
(function($) {
	...
	PRODUCTT0ORDER.prototype = {
		...
		pageInit:function(){
			...
			_this.initProfit(_this.ws.annualInterestSec,parseFloat(_this.ws.purchasingAmount));
			_this.getCouponList();
			//安卓更换键盘
			if(mui.os.android){
		...
		bindEvent:function(){
			...
			$("#app_chooseCoupon").on('tap', function(){
				...
				GHUTILS.OPENPAGE({
					url:"../../html/product-t0/product-t0-coupon.html",
					id:GHUTILS.PAGESID.PROT0COUPON,
					extras:{
		...
		switchFind: function(){
			...
			GHUTILS.OPENPAGE({
				...
				extras: {
					...
					pagesid: GHUTILS.PAGESID.PROT0ORDER,
			...
			GHUTILS.LOAD({
				...
				callback: function(result) {
					if(GHUTILS.checkErrorCode(result,_this.tips)){
						...
						if(result.maxSaleVolume == result.lockCollectedVolume || _this.ws.maxSaleVolume < result.investMin){
