...
(function($) {
	...
	ACCOUNT.prototype = {
		...
		addItem: function(tradeObj, ifAvailable) {
			var html = "";
			if(ifAvailable){
				html = '<div class="app_user_coupon_box on app_clearfix" data-oid="'+tradeObj.oid+'">';
			}else{
		...
		itemsObj: function(tradeObj, ifAvailable) {
			...
			if(ifAvailable){
				if(JSON.stringify(_this.ws.checkedCoupon) != '{}' && _this.ws.checkedCoupon.oid == tradeObj.oid){
