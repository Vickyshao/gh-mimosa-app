...
(function($) {
	...
	PRODUCTTNLIST.prototype = {
		...
		pageInit:function(){
			...
			document.addEventListener("sortTN", function(e) {
			...
			document.addEventListener("loadData", function(e) {
	      		_this.param.page = 1;
//	      		var tasteCouponObj = GHUTILS.getTasteCoupon();//获取用户体验金
		...
		bindEvent:function(){
			...
			$(".app_btn_hide").on("tap",function(){
				_this.showMenu('hide');
				mui.fire(plus.webview.getWebviewById(GHUTILS.PAGESID.PRTTN),"closeAfilter");
			});
...
//						_this.getProgress()
		...
		getProdHtml: function(tradeObj){
			...
			if(tradeObj.state != "RAISING" || tradeObj.raisedTotalNumber == GHUTILS.Fadd(tradeObj.collectedVolume, GHUTILS.Fmul(tradeObj.lockCollectedVolume, 1)) || GHUTILS.Fsub(GHUTILS.Fsub(tradeObj.raisedTotalNumber, tradeObj.collectedVolume), GHUTILS.Fmul(tradeObj.lockCollectedVolume, 1)) < tradeObj.investMin){
				soldOut = " sold_out";
...
//							+ tradeObj.durationPeriodDays +'</span>天</div><p class="app_f12 app_cb3a app_mt6 app_ca6">投资期限</p></div></div><div class="app_product_ihview app_fl mui-col-xs-6">'+ progress +'</div></div></div>'
				...
				prodText = '<div class="app_product_ihbox'+detailtype+soldOut+'" data-oid="'+tradeObj.productOid+'">'
							...
							+ tradeObj.name + '</div><div class="app_product_ihcon app_clearfix"><div class="app_product_ihview app_fl app_pt5 mui-col-xs-5"><div class="app_cmain "><span class="app_f20">'
							+ aniNum + '%</span></div><p class="app_f14 app_cb3a app_mt8 app_c8">预期年化收益率</p></div><div class="app_product_ihview app_fl mui-col-xs-7 app_pt5 app_pb10 app_f14 app_c4 text-align-right"><span class="app_c8">起投金额：</span>'
